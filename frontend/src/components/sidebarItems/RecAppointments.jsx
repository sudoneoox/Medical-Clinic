...
const AppointmentsList = ({ data = [] }) => {
  ...
  const [isAppointmentsFormVisible, setAppointmentsFormVisible] = useState(false);
  ...
  const fetchAppointmentsRecords = async (patientId) => {
    ...
    try {
      const response = await api.post(
        `${API.URL}/api/users/recappointments/${patientId}`
      );
            ...
            <table className="min-w-full bg-white border border-gray-200">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="py-2 px-4 border-b border-gray-200 text-left">Doctor Last Name</th>
                  <th className="py-2 px-4 border-b border-gray-200 text-left">Reason</th>
              ...
              <tbody>
                {appointmentRecords.map((record, index) => (
                  <tr key={index} className="hover:bg-gray-50">
                    ...
                    <td
                      className={`py-2 px-4 border-b border-gray-200 font-bold ${record.status === "COMPLETED" || record.status === "CONFIRMED"
                          ? "text-green-600"
                          : "text-red-600"
                        }`}
                    >
